...
class KwhBot {
    ...
    setupCommands() {
        ...
        // Quick payments access - MIGLIORATO E FIXATO
...
// Start the bot
const bot = new KwhBot();.answerCbQuery();
        ...
        // === FIX PRINCIPALE PER I PAGAMENTI ===
        this.bot.action(/^pay_tx_(\d+)$/, async (ctx) => {
            ...
            // IMPORTANTE: Salva l'ID nella sessione per il pulsante di conferma
        ...
        // Payment confirmation actions - FIXED COMPLETO
        this.bot.action('payment_completed', async (ctx) => {
            await ctx.answerCbQuery();
            
            // Controlla prima nella sessione (modo più affidabile)
            let transactionId = ctx.session.currentTransactionId;
            
            // Se non c'è nella sessione, cerca nel messaggio
            if (!transactionId) {
                const messageText = ctx.callbackQuery.message.text || '';
